#################################################################################
# Functions required for Bipartite Network app
# Aaron Greenville
#
################################################################################

# modified visweb function to print plot title

visweb2 <- function (web, type = "nested", main = "", prednames = TRUE, preynames = TRUE, 
          ...
          max.digits = 4, NA.col = "red") 
{
  ...
  par(mai = c(m.predsize * 0.4 * labsize * clratio + 0.2, m.preysize *
                0.4 * labsize * clratio + 0.2, 0.5, 0)) # added some margin to the top (0.5)
  plot(1, type = "n", main = main, axes = FALSE, xlim = c(0, n.pred), # added main back in
       ylim = c(0, n.prey), asp = 1, xlab = xlabel, ylab = ylabel)
    ...
    for (ii in 1:n.pred) {
      if (substr(square, 1, 1) == "c") {
        ...
        if (substr(square, 1, 1) == "i") {
          c = (1 - (which(round(lev, 6) == round(web[n.prey - 
                                                       i + 1, ii], 6)) - 1)/nl)
        }
        else {
      ...
      if (substr(square, 1, 1) == "d") 
        c = def.col[which(round(lev, 6) == round(web[n.prey - 
                                                       i + 1, ii], 6))]
